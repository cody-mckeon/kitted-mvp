...
export default async function ProductPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ source?: string; catalogError?: string; addError?: string }> }) {
  const [{ id }, { source = "direct", catalogError, addError }] = await Promise.all([params, searchParams]);
  ...
  return <main className="product-detail-page">
    ...
    <article className="product-detail">
      ...
      <div className="product-summary">
        ...
        <AddToCart product={{ id: product.id, name: product.name, category: product.category, price: product.price }} available={available} source={source} simulateFailure={addError === "1"} />
      </div>
