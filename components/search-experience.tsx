...
export function SearchExperience({ initialQuery, simulateError }: { initialQuery: string; simulateError: boolean }) {
  ...
  return <main className="search-page">
    ...
    {state === "results" && <section className="search-results" aria-live="polite"><div className="results-heading"><h2>Results for “{submittedQuery}”</h2><p>{products.length} {products.length === 1 ? "product" : "products"}</p></div><div className="product-grid">{products.map((product) => <ProductCard product={product} key={product.id} />)}</div></section>}
