...
export function ProductListing({ products, heading }: { products: Product[]; heading: string }) {
  ...
  return <section className="products-section catalog-listing" aria-live="polite">
    ...
    <div className="listing-layout"><aside className={`filters-panel ${mobileOpen ? "mobile-open" : ""}`} aria-label="Product filters"><div className="filter-header"><h2>Filters</h2><button type="button" onClick={clear} disabled={!activeCount}>Clear all</button><button className="filter-close" type="button" aria-label="Close filters" onClick={() => setMobileOpen(false)}>×</button></div>{controls}<button className="button filter-done" type="button" onClick={() => setMobileOpen(false)}>Show {results.length} results</button></aside><div className="listing-results"><div className="results-heading"><h2>{heading}</h2><p>{results.length} {results.length === 1 ? "product" : "products"}</p></div>{results.length ? <div className="product-grid">{results.map((product) => <ProductCard product={product} key={product.id} />)}</div> : <div className="zero-results"><span className="state-icon" aria-hidden="true">⌁</span><h3>No products match</h3><p>Try removing a filter or reset them all to see the full collection.</p><button className="button" type="button" onClick={clear}>Reset filters</button></div>}</div></div>
  </section>;
}
