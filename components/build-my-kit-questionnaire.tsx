...
import { activities, type Product } from "@/lib/catalog";
...
export function BuildMyKitQuestionnaire() {
  ...
  if (completed && result) {
    ...
    return <main className="kit-results-page">
      <header className="kit-results-hero"><p className="kicker">Based on your trip details</p><h1>{state === "available-kit" ? "Your recommended starting kit" : state === "partial-kit" ? "Your partial starting kit" : "No suitable starting kit found"}</h1><p>{state === "available-kit" ? "We found an available match for every category in this starting kit." : state === "partial-kit" ? "This kit is partial because an item was removed or a category has no suitable available product." : "The current catalog does not have suitable available matches for these trip details."}</p><ul aria-label="Trip criteria">{criteria.map((criterion) => <li key={criterion}>{criterion}</li>)}<li>{completed.trip.groupSize} {completed.trip.groupSize === 1 ? "person" : "people"}</li><li>{labels[completed.shopper.experienceLevel]}</li><li>{labels[completed.shopper.purchasePriority]} priority</li></ul><div className="kit-actions">{activeKit.length > 0 && <button className="button" type="button" onClick={addKit}>Add current kit to cart</button>}<button className="button secondary-button" type="button" onClick={editTrip}>Edit trip details &amp; regenerate</button><button className="back-button" type="button" onClick={reset}>Start over</button></div>{cartFeedback && <p className={`kit-cart-feedback ${cartFeedbackKind}`} role="status" aria-live="polite">{cartFeedback}</p>}</header>
      ...
      {result.recommendations.length > 0 && <section className="kit-products" aria-labelledby="kit-products-title"><div className="results-heading"><div><p className="kicker">Recommended gear</p><h2 id="kit-products-title">{activeKit.length} {activeKit.length === 1 ? "product" : "products"} in your current kit</h2></div><p>A focused starting point—not a claim of everything you need.</p></div><div className="kit-product-grid">{result.recommendations.map((recommendation) => { const alternatives = getRecommendationAlternatives(completed, recommendation); const specification = keySpecification(recommendation.product); const isOpen = alternativesFor === recommendation.category; return <div className="kit-recommendation" key={recommendation.category}><ProductCard product={recommendation.product} source="recommended-kit" onSelect={() => selectProduct(recommendation.product.id, recommendation.category, "recommended-product")} /><div className="recommendation-reason"><strong>Why it fits</strong><p>{recommendation.reason}</p>{specification && <p className="key-spec"><strong>Key spec</strong> {specification}</p>}</div><div className="recommendation-controls"><button className="button" type="button" onClick={() => addRecommendation(recommendation.product)}>Add this product</button><button className="back-button" type="button" onClick={() => removeRecommendation(recommendation)}>Remove from kit</button>{alternatives.length > 0 && <button type="button" aria-expanded={isOpen} onClick={() => { setAlternativesFor(isOpen ? null : recommendation.category); if (!isOpen) track("recommendation_alternative_viewed", { product_id: recommendation.product.id, recommendation_category: recommendation.category, alternative_product_ids: alternatives.map(({ product }) => product.id).join(","), alternative_count: alternatives.length, ...analyticsContext }); }}>View alternatives ({alternatives.length})</button>}</div>{isOpen && <div className="kit-alternatives"><strong>Choose an alternative</strong>{alternatives.map((alternative) => <article key={alternative.product.id}><ProductCard product={alternative.product} source="recommended-kit-alternative" onSelect={() => selectProduct(alternative.product.id, recommendation.category, "alternative-product")} />{keySpecification(alternative.product) && <p className="alternative-spec"><strong>Key spec</strong> {keySpecification(alternative.product)}</p>}<p>{alternative.reason}</p><button className="button" type="button" onClick={() => replaceRecommendation(recommendation, alternative)}>Replace with {alternative.product.name}</button></article>)}</div>}{alternatives.length === 0 && <p className="no-alternatives">No other suitable available products match this category and trip.</p>}</div>; })}</div></section>}
